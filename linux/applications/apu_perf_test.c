 ...
 * 
 * This is a Linux userspace app that runs on the APU (Cortex-A53) and sends
 * data packets to the RPU through shared memory while measuring transfer times.
 * 
...
/* Packet sizes we'll test (in bytes) */
...
/* Result structure (needs to match what the RPU uses) */
typedef struct {
...
/**
 * Map physical memory region using /dev/mem
 */
static int map_memory(void)
{
    ...
    // Results area is just an offset within the shared memory
...
/**
 * Read TTC0 Timer 0 counter value
 */
...
/**
 * Wait for RPU to signal it's ready
 */
...
/**
 * Send a single packet to the RPU
 */
static int send_packet(uint32_t size, uint8_t *payload)
{
    uint32_t ts;
    
    // Copy the payload data into shared memory if we have any
    if (payload && size > 0) {
        memcpy((void *)&shared_mem[4], payload, size);
    }
    
    // Write packet metadata (size goes in word 1)
    shared_mem[1] = size;
    
    // Grab timestamp right before we signal the RPU
    ts = read_timer();
    shared_mem[2] = ts;
    shared_mem[3] = 0;  /* Reserved for future use */
    ...
    // Wait for the RPU to acknowledge (10ms timeout should be plenty)
...
static int run_experiment(int iterations_per_size, const char *output_file)
{
    ...
    // Allocate buffer for the largest packet size
    ...
    // Make sure the RPU is actually ready before we start
    ...
    for (size_idx = 0; size_idx < NUM_SIZES; size_idx++) {
        ...
        // Send multiple iterations for statistical significance
        for (iter = 0; iter < iterations_per_size; iter++) {
            ...
            // Small delay between packets to avoid overwhelming the RPU
            usleep(100);  /* 100us */
        }
